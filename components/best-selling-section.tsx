...
function toLiveProduct(p: BestSellingProduct): LiveProduct {
  return {
...
const PERIOD_LABELS: Record<1 | 2 | 3, string> = {
  [BEST_SELLING_PERIOD.Weekly]: "Haftanın En Çok Satanları",
  [BEST_SELLING_PERIOD.Monthly]: "Ayın En Çok Satanları",
  [BEST_SELLING_PERIOD.Yearly]: "Yılın En Çok Satanları",
};

export function BestSellingSection({
  period = BEST_SELLING_PERIOD.Weekly,
  limit = 10,
  ...
  const query = useGetBestSellingProducts(period);

  const products = (query.data?.products ?? [])
    .filter((p): p is BestSellingProduct => Boolean(p))
    .slice(0, limit)
    .map(toLiveProduct);
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-3xl border border-dashed border-border bg-card/50 px-6 py-12 text-center">
      ...
      <p className="max-w-sm text-xs text-muted-foreground">
        Yeterli sipariş alındığında en çok satanlar burada listelenecek.
      </p>
    </div>
  );
}
