...
import { useStartCheckout } from "./_services/queries";
...
export default function CheckoutPage() {
  ...
  const handlePay = async () => {
    ...
    // Open the modal immediately so the user gets instant feedback while we
    // wait for the Iyzico checkout form to be prepared. The modal will show
    // a loading state until `checkoutFormHtml` is set below.
    ...
    try {
      ...
      try {
        ...
        if (ipResponse.ok) {
          const data = (await ipResponse.json()) as { ip?: string | null };
      ...
      const response = await initializeCheckout.mutateAsync({
        body: {
          ...
          notes: items
            .map(
              (it) =>
                `${it.productName ?? `Ürün #${it.productId}`} x${it.quantity}`,
            )
            .join(", "),
          clientIp,
        },
  ...
  const handlePaymentModalChange = (open: boolean) => {
    // Block the modal from being dismissed while the Iyzico checkout form
    // is being prepared. Otherwise the user could close it mid-flight and
    // miss the rendered form when it eventually loads.
    if (isPreparingCheckout && !open) return;
    setShowPaymentModal(open);
    if (!open) {
      // Reset the form HTML so the next payment attempt re-prepares the
      // checkout form from scratch.
